...
import { guardarEstProy } from '../../../../api/estadosAPI';


AddEstProyForm.propTypes = {
    reset: PropTypes.func,
    onClose: PropTypes.func
}

export default function AddEstProyForm({ reset, onClose }) {
    const { auth } = useAuth();
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, actions) => {
            try {
                const response = await guardarEstProy(values, auth?.token);
